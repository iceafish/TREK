...
```bash
pnpm run build              # tsdown → dist/ (CJS + ESM + .d.ts). REQUIRED before server/client typecheck or run
pnpm run build:watch        # tsdown --watch --no-clean (the root `pnpm run dev` runs this)
pnpm run typecheck          # tsc --noEmit
pnpm run lint               # eslint --fix src/**/*.ts
pnpm run test               # vitest run
pnpm run i18n:parity        # audit locale drift, exits 0 (prints a per-locale report)
pnpm run i18n:parity:strict # CI gate — exits 1 on any drift
```
...
**Always rebuild after changing a schema or locale** — the consumers import the compiled `dist/`, not `src/`. If a server/client typecheck fails to see a new export, you probably forgot `pnpm run build` here.
...
**`src/plugin-permissions.ts` is machine-generated** (by `server/scripts/gen-plugin-facts.ts` from the server's plugin protocol) — never hand-edit it; regenerate with `pnpm run gen:plugin-facts` from `server/`, where `check:plugin-facts` is the CI drift gate.
