...
```bash
pnpm run dev               # Vite dev server; proxies /api,/ws,/uploads,/oauth → http://localhost:3001
pnpm run build             # prebuild generates PWA icons, then vite build
pnpm run typecheck         # tsc --noEmit
pnpm run lint              # eslint .
pnpm run lint:pages        # enforce the Page pattern (scripts/check-page-pattern.mjs)
pnpm run test              # vitest run (tests/** + src/**/*.test.{ts,tsx})
pnpm run test:unit         # tests/unit
pnpm run test:integration  # tests/integration + co-located src tests
pnpm run test:coverage     # v8 coverage
pnpm run e2e               # Playwright
pnpm run e2e:report        # open last Playwright report
pnpm run shots             # Playwright screenshots project (shots:promote to accept)
pnpm run theme:lint        # theme conformance audit (theme:lint:strict exits 1 on findings; not a CI gate)
```
...
The dev server needs the API on **:3001** — run the server (`pnpm run dev` from root or `server/`) alongside, or `pnpm run dev` at the repo root to start both.
