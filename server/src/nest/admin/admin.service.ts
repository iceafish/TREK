...
export class AdminService {
  ...
  async getGithubReleases(perPage: string = '10', page: string = '1') {
    ...
    const data = await this.fetchGithub(`https://api.github.com/repos/${readEnv().app.repo}/releases?${qs}`);
  ...
  async checkVersion(): Promise<VersionInfo> {
    ...
    if (isPrerelease) {
      ...
      const data = await this.fetchGithub(
        `https://api.github.com/repos/${readEnv().app.repo}/releases?per_page=100`,
      ) as
      ...
      const data = await this.fetchGithub(
        `https://api.github.com/repos/${readEnv().app.repo}/releases/latest`,
      ) as
