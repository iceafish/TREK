...
```bash
pnpm run dev               # scripts/dev.mjs: tsc -w then node --watch dist/index.js (server on :3001)
pnpm run build             # scripts/build.mjs — see "emit-anyway" note below
pnpm run typecheck         # tsc --noEmit (the real type gate; build does NOT fail on type errors)
pnpm run typecheck:tests   # tsc over tests/ — CI runs this too; vitest green does NOT mean typed mocks compile
pnpm run lint              # eslint --fix on src/apps/libs/test
pnpm run lint:check        # eslint, no fix
pnpm run test              # vitest run (all of tests/**)
pnpm run test:unit         # tests/unit
pnpm run test:integration  # tests/integration
pnpm run test:ws           # tests/websocket
pnpm run test:e2e          # tests/e2e (boots Nest modules against a temp in-memory SQLite)
pnpm run test:coverage     # istanbul coverage; per-domain ratchet over src/nest/** (floor ≥80%)
pnpm run gen:plugin-facts  # regenerate the plugin-protocol tables into plugin-sdk/ + shared/
pnpm run check:plugin-facts # CI gate — fails if the generated copies drifted from envelope.ts
```
...
**Build emits even with type errors** (`scripts/build.mjs` catches `tsc` failures and warns). So `pnpm run build` succeeding does **not** mean the code typechecks — always run `pnpm run typecheck` to gate types.
